...
class DeepSORTTracker(DetectionBasedObjectTracker):
    ...
    def track(self, frame, frame_idx:int) -> List[Track]:
        ...
        active_tracks = [self.to_dna_track(ds_track, frame_idx) for ds_track in tracker.tracks]
        deleted_tracks = [self.to_dna_track(ds_track, frame_idx) for ds_track in deleted_tracks]
        return active_tracks + deleted_tracks

    def to_dna_track(self, ds_track: DSTrack, frame_idx: int) -> Track:
